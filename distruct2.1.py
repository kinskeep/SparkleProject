...
def plot_admixture(admixture, population_indices, population_labels, title):
    ...
    colors = ('darkorchid','coral','turquoise','limegreen','deeppink','blue','green','red','black') # Color choice!  Disable if you enable above line.
    text_color = 'k'
    bg_color = 'w'
    fontsize = 10
...
#        subplot.bar(lefts, admixture[:,k], width=indiv_width, bottom=bottoms, facecolor=colors[k], edgecolor=colors[k], linewidth=0.2)
    ...
    for p,popname in enumerate(population_labels):
        ...
        if indices.size>0:
            ...
            subplot.axvline(vline_pos, linestyle='-', linewidth=1, c='#000000')
            ...
            label_position = (xmin+(2*indices.min()+indices.size)*0.5*indiv_width, ymin-0.03)
